...
| `src/components/ui-client.tsx` | `ConfirmAction` (الإقرارُ لا «هل أنت متأكّد؟») · `toast({ undo })` والعارضُ الواحد في القشرة — التراجعُ حيث يسمح المجال وبعد تأكيد الخادم · `Sheet` (حوارٌ أصليّ، ورقةٌ على الجوّال) · `TableFilter` |
