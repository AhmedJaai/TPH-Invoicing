      ...
      {message ? (
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <select
            ...
            className="min-h-11 min-w-[9rem] rounded-lg border border-line-input bg-raised px-2.5 text-xs sm:min-h-9"
          >
          ...
          {creating && (
            <>
              <input
                ...
                dir="auto"
                className="min-h-11 min-w-[9rem] flex-1 rounded-lg border border-line-input bg-raised px-2.5 text-xs sm:min-h-9"
              />
              <select
                aria-label="فئة الصنف"
                value={category}
                onChange={(e) => setCategory(e.target.value as ProductCategory)}
                className="min-h-11 rounded-lg border border-line-input bg-raised px-2.5 text-xs sm:min-h-9"
              >
                {CATEGORIES.map((c) => (
